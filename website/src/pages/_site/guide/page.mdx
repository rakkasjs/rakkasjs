...
Rakkas stands on the shoulder of React and Vite. They bring the following to the table:
...
You can use more than one dynamic parameter like `/user/[userName]/posts/[postId]`. You can even put multiple parameters in a single path segment if you delimit them with `-` or `.`, like `/user-[name]-[surname]/contact-info` (matches `/user-fatih-aygun/contact-info` for instance).
...
By default, Rakkas only calls this function again (we'll call it **"reloadin"**) when the URL path or the query string changes. You can force a reload by calling the `reload` prop:

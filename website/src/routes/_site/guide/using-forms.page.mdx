...
The traditional way of mutating server-side data is to use HTML forms. Rakkas has first-class support for forms via [Remix](https://remix.run/)-inspired **action handlers**. If you export an `action` function from your pages, Rakkas will call it when a `POST` request is made to the page's route. You can either redirect to another page by returning `{ redirect: '/path/to/page' }` or return some data (`{ data: 'some data' }`). Returned data will be available on pages and layouts in `props.actionData`. Usually you will redirect on success and return some error message on failure.

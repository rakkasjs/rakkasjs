...
<!-- (TODO: Uncomment when implemented) `navigate` returns a promise that resolves to true when the navigation successfully completes and false if it is aborted (due to another call to navigate, for example). If `navigate` is called with a link that causes a full reload, the promise never returns. -->

You can use `history.back()`, `history.forward()` and `history.go()` to navigate back and forward in the browser history but _you can't_ use `history.pushState()` or `history.replaceState()` because Rakkas would have no way of knowing they were called.

...
- Install the release candidate packages for React 18 (`npm install -S react@rc react-dom@rc`).
...
- Use the `Head` component from `rakkasjs` instead of `Helmet` from `react-helmet-async`. The latter is still supported for now but may be removed in the near future.
- `definePage` and `defineLayout` don't exist anymore:
  ...
  - Use the `ClientSide` component to opt out of SSR.

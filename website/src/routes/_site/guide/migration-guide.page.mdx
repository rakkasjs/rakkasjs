...
- `definePage` and `defineLayout` don't exist anymore. Replace them with (depending on the use case)
  ...
  - `useQuery` calls
  - `preload` functions
  - `Redirect` or `ResponseHeaders` components
...
- Consider using `useServerSideQuery`, `useServerSideMutation`, and page `action` functions instead of API routes for data fetching.

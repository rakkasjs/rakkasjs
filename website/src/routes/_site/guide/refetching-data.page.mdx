...
`useQuery` and `useServerSideQuery` return a `QueryResult` object. Apart from the `data` property, it also has a `refetch` method that can be used to refetch the data.

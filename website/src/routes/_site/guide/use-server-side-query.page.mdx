...
First of all, `useServerSideQuery` is not a normal function. It's more like a macro that is processed by Rakkas's code transforms. The function argument must be defined inline, passing functions defined elsewhere (`useSSQ(someFunc)`) will not work.{/* TODO: createServerSideFunction */}
...
One very handy use of `useSSQ` is to easily circumvent CORS restrictions when calling third-party APIs:

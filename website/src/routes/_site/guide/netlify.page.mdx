...
Rakkas projects can be deployed to [Netlify Functions](https://www.netlify.com/products/functions/) or [Netlify Edge Functions](https://docs.netlify.com/netlify-labs/experimental-features/edge-functions/). To do that, first, you should set the `adapter` option of Rakkas Vite plugin to `"netlify"` or `"netlify-edge"`:
...
After building with `rakkas build`, you can preview your app with `netlify dev` or publish with `netlify deploy`.

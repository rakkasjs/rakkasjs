...
> **NOTE:** Route config files _must_ be in vanilla JavaScript. TypeScript, JSX, and other formats that require compilation are not supported. You can use `@ts-check` and `@type` comments like in the examples below to get type-checking.

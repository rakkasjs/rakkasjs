...
Rakkas uses [`react-helmet-async`](https://github.com/staylor/react-helmet-async#readme) for managing head tags like `title`. The `Head` component is simply `Helmet` reexported, check their documentation for usage examples.

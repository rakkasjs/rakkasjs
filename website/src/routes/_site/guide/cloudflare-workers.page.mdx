...
Rakkas projects can be deployed to Cloudflare Workers. To do that, first, you should set the `adapter` option of Rakkas Vite plugin to `"cloudflare-workers"`:
...
After building with `rakkas build`, you can publish your project with `wrangler publish`. You can also test locally with `miniflare --modules dist/cloudflare-workers-bundle/index.js`.

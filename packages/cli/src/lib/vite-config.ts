...
export async function makeViteConfig(
	...
	const result: InlineConfig & { ssr: SSROptions } = {
		...
		define: {
			...
			RAKKAS_DEFAULT_LOCALE: JSON.stringify(config.locales?.default || "en"),
			RAKKAS_DETECT_LOCALE: JSON.stringify(!!config.locales?.detect),
			RAKKAS_LOCALE_COOKIE_NAME: JSON.stringify(config.locales?.cookieName),
		},

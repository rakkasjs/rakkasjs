import { Head, Link } from "@rakkasjs/core";
...
If you prefer a manual setup, you can install `@rakkasjs/cli` as a dev dependency, and `@rakkasjs/core` and `@rakkasjs/runner-node` as production dependencies:

```sh
npm install --save-dev @rakkasjs/cli
npm install --save @rakkasjs/core @rakkasjs/runner-node
```
...
Page components receive some useful props (TypeScript definitions are available as `RakkasComponentProps` in `@rakkasjs/core` package):

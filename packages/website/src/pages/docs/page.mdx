...
Apart from all the awesomeness brought by React and Vite, it has some interesting features on its own right:
...
If you prefer a manual setup, you can install `rakkasjs` and `@rakkasjs/runner-node` as production dependencies, and `@rakkasjs/cli` as a dev dependency:
